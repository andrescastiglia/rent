...
export default function LeaseDetailScreen() {
  ...
  return (
    <Screen scrollViewTestID="leaseDetail.scroll">
      ...
      {leaseQuery.error ? (
        <Text style={styles.error}>{(leaseQuery.error as Error).message}</Text>
      ) : null}
      ...
      {lease ? (
        <>
          ...
          <View style={styles.card}>
            ...
            {personEmail ? (
              <Text style={styles.detail}>{personEmail}</Text>
            ) : null}
            {personPhone ? (
              <Text style={styles.detail}>{personPhone}</Text>
            ) : null}
          </View>
          ...
          <View style={styles.card}>
            ...
            {lease.contractType === 'rental' ? (
              <>
                <Text
                  style={styles.detail}
                >{`${t('leases.rentAmount')}: ${formatMoney(lease.rentAmount, lease.currency)}`}</Text>
                <Text
                  style={styles.detail}
                >{`${t('leases.securityDeposit')}: ${formatMoney(lease.depositAmount, lease.currency)}`}</Text>
              </>
            ) : (
              <Text
                style={styles.detail}
              >{`${t('leases.fields.fiscalValue')}: ${formatMoney(lease.fiscalValue, lease.currency)}`}</Text>
            )}
          </View>
          ...
          {lease.contractType === 'rental' ? (
            <View style={styles.card}>
              ...
              <Text
                style={styles.detail}
              >{`${t('leases.startDate')}: ${formatDate(lease.startDate)}`}</Text>
              <Text
                style={styles.detail}
              >{`${t('leases.endDate')}: ${formatDate(lease.endDate)}`}</Text>
            </View>
          ) : null}
          ...
          <View style={styles.actions}>
            ...
            <AppButton
              ...
              onPress={() => {
                Alert.alert(
                  t('leases.deleteLease'),
                  t('leases.confirmDelete'),
                  [
