...
export default function SalesPage() {
  ...
  const [receiptForm, setReceiptForm] = useState<Record<string, CreateSaleReceiptInput>>({});
  ...
  const handleCreateReceipt = async (agreementId: string) => {
    ...
    try {
      ...
      setReceiptForm((prev) => ({
        ...prev,
        [agreementId]: { amount: 0, paymentDate: new Date().toISOString().split('T')[0] },
      }));
    } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-6">
          <form onSubmit={handleCreateFolder} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('folders.new')}</h2>
            <input
              type="text"
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">{t('folders.list')}</h2>
            <div className="space-y-2">
              {folders.map((folder) => (
                <div key={folder.id} className="rounded-md border border-gray-200 dark:border-gray-700 p-3">
              ...
              {folders.length === 0 && (
                <p className="text-sm text-gray-500 dark:text-gray-400">{t('folders.empty')}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <form onSubmit={handleCreateAgreement} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('agreements.new')}</h2>
            <select
              value={agreementForm.folderId}
              onChange={(e) => setAgreementForm((prev) => ({ ...prev, folderId: e.target.value }))}
              className="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-2 text-sm"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input
                ...
                onChange={(e) => setAgreementForm((prev) => ({ ...prev, buyerName: e.target.value }))}
                className="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-2 text-sm"
              />
              <input
                type="text"
                ...
                onChange={(e) => setAgreementForm((prev) => ({ ...prev, buyerPhone: e.target.value }))}
                className="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-2 text-sm"
              />
              <input
                type="number"
                ...
                onChange={(e) => setAgreementForm((prev) => ({ ...prev, totalAmount: Number(e.target.value) }))}
                className="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-2 text-sm"
              />
              <input
                type="number"
              ...
              <input
                ...
                onChange={(e) => setAgreementForm((prev) => ({ ...prev, installmentCount: Number(e.target.value) }))}
                className="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-2 text-sm"
              />
              <input
                type="date"
          ...
          <div className="space-y-4">
            {agreements.map((agreement) => {
              ...
              const balance = Number(agreement.totalAmount) - Number(agreement.paidAmount);
              return (
                <div key={agreement.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-white">{agreement.buyerName}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{agreement.buyerPhone}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{t('agreements.balance')}: {balance.toLocaleString()}</p>
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      {t('agreements.installments')}: {agreement.installmentAmount.toLocaleString()} x {agreement.installmentCount}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                      ...
                      onChange={(e) =>
                        setReceiptForm((prev) => ({
                          ...
                          [agreement.id]: {
                            ...
                            paymentDate: prev[agreement.id]?.paymentDate || new Date().toISOString().split('T')[0],
                          },
                        }))
                      }
                      className="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-2 text-sm"
                    ...
                    <input
                      type="date"
                      value={receiptForm[agreement.id]?.paymentDate || new Date().toISOString().split('T')[0]}
                      onChange={(e) =>
                        setReceiptForm((prev) => ({
                          ...prev,
                          [agreement.id]: {
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400">{t('receipts.duplicate')}</span>
                  </div>

                  <div className="mt-4 space-y-2">
                    {agreementReceipts.map((receipt) => (
                      <div key={receipt.id} className="flex flex-col md:flex-row md:items-center justify-between text-sm text-gray-600 dark:text-gray-300 border-t border-gray-200 dark:border-gray-700 pt-2">
                        <span>{receipt.receiptNumber}</span>
                        <span>{new Date(receipt.paymentDate).toLocaleDateString()}</span>
                        <span>{t('receipts.overdue')}: {receipt.overdueAmount.toLocaleString()}</span>
                        <span>{t('receipts.balanceAfter')}: {receipt.balanceAfter.toLocaleString()}</span>
                        {receipt.pdfUrl && (
                          <a
                    ...
                    {agreementReceipts.length === 0 && (
                      <p className="text-xs text-gray-500 dark:text-gray-400">{t('receipts.empty')}</p>
                    )}
                  </div>
                </div>
              );
