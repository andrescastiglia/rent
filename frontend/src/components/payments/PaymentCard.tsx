...
export function PaymentCard({ payment }: PaymentCardProps) {
    ...
    >(null);
    const [linkedCreditNote, setLinkedCreditNote] = useState<CreditNote | null>(null);
    ...
    useEffect(() => {
        ...
        const loadCreditNote = async () => {
            ...
            try {
                const creditNotes = await invoicesApi.listCreditNotes(payment.invoiceId);
                if (cancelled) return;
                const current =
                    creditNotes.find((item) => item.paymentId === payment.id) ??
                    null;
