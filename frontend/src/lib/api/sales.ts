...
export const salesApi = {
    ...
    createFolder: async (data: CreateSaleFolderInput): Promise<SaleFolder> => {
        ...
        const token = getToken();
        return apiClient.post<SaleFolder>('/sales/folders', data, token ?? undefined);
    },
    ...
    getAgreements: async (folderId?: string): Promise<SaleAgreement[]> => {
        ...
        return apiClient.get<SaleAgreement[]>(`/sales/agreements${query}`, token ?? undefined);
    },

    createAgreement: async (data: CreateSaleAgreementInput): Promise<SaleAgreement> => {
        if (IS_MOCK_MODE) {
            await delay(DELAY);
        ...
        const token = getToken();
        return apiClient.post<SaleAgreement>('/sales/agreements', data, token ?? undefined);
    },
    ...
    getReceipts: async (agreementId: string): Promise<SaleReceipt[]> => {
        ...
        const token = getToken();
        return apiClient.get<SaleReceipt[]>(`/sales/agreements/${agreementId}/receipts`, token ?? undefined);
    },

    createReceipt: async (agreementId: string, data: CreateSaleReceiptInput): Promise<SaleReceipt> => {
        if (IS_MOCK_MODE) {
            await delay(DELAY);
            const now = Date.now();
            ...
            MOCK_RECEIPTS[agreementId] = [receipt, ...(MOCK_RECEIPTS[agreementId] ?? [])];
            return receipt;
        }
        const token = getToken();
        return apiClient.post<SaleReceipt>(`/sales/agreements/${agreementId}/receipts`, data, token ?? undefined);
    },
};
