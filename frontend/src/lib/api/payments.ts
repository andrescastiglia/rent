...
export const paymentsApi = {
  ...
  getById: async (id: string): Promise<Payment | null> => {
    if (shouldUseMock()) {
      ...
      const account = MOCK_TENANT_ACCOUNTS.find(
        (a) => a.id === payment.tenantAccountId,
      );
...
export const invoicesApi = {
  ...
  listCreditNotes: async (invoiceId: string): Promise<CreditNote[]> => {
    ...
    const token = getToken();
    return apiClient.get<CreditNote[]>(
  ...
  downloadCreditNotePdf: async (
    ...
    const response = await fetch(
      `${API_URL}/invoices/credit-notes/${creditNoteId}/pdf`,
      {
        method: "GET",
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      },
...
export const tenantAccountsApi = {
  ...
  getBalance: async (accountId: string): Promise<AccountBalance> => {
    ...
    return apiClient.get<AccountBalance>(
      `/tenant-accounts/${accountId}/balance`,
      token ?? undefined,
    );
  },
  ...
  getMovements: async (accountId: string): Promise<TenantAccountMovement[]> => {
    ...
    const token = getToken();
    return apiClient.get<TenantAccountMovement[]>(
      `/tenant-accounts/${accountId}/movements`,
      token ?? undefined,
    );
  },
