...
export class InvoicesService {
    ...
    async create(dto: CreateInvoiceDto): Promise<Invoice> {
        ...
        // Calcular total
        const total =
            Number(dto.subtotal) + Number(dto.lateFee || 0) + Number(dto.adjustments || 0);

        // Generar número de factura si no se proporciona
        const invoiceNumber = dto.invoiceNumber || (await this.generateInvoiceNumber(ownerId));
    ...
    async findAll(filters: {
        ...
        query.orderBy('invoice.issued_at', 'DESC').skip((page - 1) * limit).take(limit);
     ...
     * @returns Número de factura
     */
    private async generateCommissionInvoiceNumber(companyId: string): Promise<string> {
