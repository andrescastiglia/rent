...
import { Lease, AdjustmentType, InflationIndexType } from '../leases/entities/lease.entity';
import { InflationIndex, InflationIndexType as IndexTypeEntity } from './entities/inflation-index.entity';
import { TenantAccountsService } from './tenant-accounts.service';

describe('InvoicesService', () => {
  let service: InvoicesService;
  let invoicesRepository: MockRepository<Invoice>;
  let commissionRepository: MockRepository<CommissionInvoice>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(CommissionInvoice), useValue: createMockRepository() },
        { provide: getRepositoryToken(Lease), useValue: createMockRepository() },
        { provide: getRepositoryToken(InflationIndex), useValue: createMockRepository() },
    ...
    commissionRepository = module.get(getRepositoryToken(CommissionInvoice));
  ...
  it('should apply late fee when requested', async () => {
    ...
    (tenantAccountsService.findByLease as jest.Mock).mockResolvedValue({ id: 'acc-1' });
    (tenantAccountsService.calculateLateFee as jest.Mock).mockResolvedValue(100);
    invoicesRepository.create!.mockImplementation((data) => data);
    invoicesRepository.save!.mockImplementation(async (data) => ({ id: 'inv-1', ...data }));
    leasesRepository.save!.mockResolvedValue(lease);

    await service.generateForLease('lease-1', { applyLateFee: true });

    expect(tenantAccountsService.calculateLateFee).toHaveBeenCalledWith('acc-1');
  ...
  it('should auto-calculate billing period and due date', async () => {
    ...
    (tenantAccountsService.findByLease as jest.Mock).mockResolvedValue({ id: 'acc-1' });
    invoicesRepository.create!.mockImplementation((data) => data);
    invoicesRepository.save!.mockImplementation(async (data) => ({ id: 'inv-2', ...data }));
    leasesRepository.save!.mockResolvedValue(lease);

    await service.generateForLease('lease-2', { applyLateFee: false });

    const created = invoicesRepository.create!.mock.calls[0][0];
    expect(new Date(created.periodStart).toISOString()).toBe('2025-01-01T00:00:00.000Z');
  ...
  it('should use Casa Propia index for inflation adjustments', async () => {
    ...
    inflationIndexRepository.findOne!.mockResolvedValue({ variationMonthly: 10 } as any);
    leasesRepository.save!.mockResolvedValue(lease);
