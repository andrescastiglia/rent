...
export class PaymentsService {
    ...
    async create(dto: CreatePaymentDto, userId?: string): Promise<Payment> {
        ...
        const account = await this.tenantAccountsService.findOne(dto.tenantAccountId);
    ...
    private async generateReceipt(payment: Payment): Promise<Receipt> {
        ...
        // Generar PDF
        try {
            const pdfUrl = await this.receiptPdfService.generate(savedReceipt, payment);
    ...
    async findAll(
        ...
        query.orderBy('payment.payment_date', 'DESC').skip((page - 1) * limit).take(limit);
