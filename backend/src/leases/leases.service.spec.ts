...
describe('LeasesService', () => {
    ...
    describe('create', () => {
        it('should create a lease in draft status', async () => {
            ...
            expect(unitRepository.findOne).toHaveBeenCalledWith({ where: { id: 'unit-1' } });
    ...
    describe('update', () => {
        ...
        it('should throw BadRequestException when updating non-draft lease', async () => {
            const updateDto = { rentAmount: 1600 };
            leaseRepository.findOne.mockResolvedValue({ ...mockLease, status: LeaseStatus.ACTIVE });

            await expect(service.update('lease-1', updateDto)).rejects.toThrow(BadRequestException);
        });
    });

    describe('activate', () => {
        it('should activate a draft lease', async () => {
            leaseRepository.findOne
                .mockResolvedValueOnce(mockLease)
                .mockResolvedValueOnce(null); // No existing active lease
            leaseRepository.save.mockResolvedValue({ ...mockLease, status: LeaseStatus.ACTIVE });
            unitRepository.update.mockResolvedValue({ affected: 1, raw: [], generatedMaps: [] });
            pdfService.generateContract.mockResolvedValue(undefined);

            const result = await service.activate('lease-1', 'user-1');

            expect(unitRepository.update).toHaveBeenCalledWith('unit-1', { status: UnitStatus.OCCUPIED });
        ...
        it('should throw ConflictException when unit has active lease', async () => {
            ...
            await expect(service.activate('lease-1', 'user-1')).rejects.toThrow(ConflictException);
        });
    });
    ...
    describe('terminate', () => {
        it('should terminate an active lease', async () => {
            ...
            leaseRepository.save.mockResolvedValue({ ...activeLease, status: LeaseStatus.TERMINATED });
            unitRepository.update.mockResolvedValue({ affected: 1, raw: [], generatedMaps: [] });

            const result = await service.terminate('lease-1', 'Early termination');

            expect(unitRepository.update).toHaveBeenCalledWith('unit-1', { status: UnitStatus.AVAILABLE });
        ...
        it('should throw BadRequestException when terminating non-active lease', async () => {
            leaseRepository.findOne.mockResolvedValue(mockLease); // DRAFT status

            await expect(service.terminate('lease-1')).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('renew', () => {
        it('should renew an active lease', async () => {
            const activeLease = { ...mockLease, status: LeaseStatus.ACTIVE };
            const newLease = { ...mockLease, id: 'lease-2', status: LeaseStatus.DRAFT };

            leaseRepository.findOne.mockResolvedValue(activeLease);
            leaseRepository.save
                .mockResolvedValueOnce({ ...activeLease, status: LeaseStatus.RENEWED })
    ...
    describe('remove', () => {
        ...
        it('should throw BadRequestException when deleting active lease', async () => {
            ...
            await expect(service.remove('lease-1')).rejects.toThrow(BadRequestException);
        });
    });
});
