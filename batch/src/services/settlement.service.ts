...
export class SettlementService {
     ...
     */
    async calculateSettlement(ownerId: string, period: string): Promise<SettlementCalculation> {
        ...
        const commissionValue =
            Number.parseFloat(owner.commission_rate) || this.defaultCommissionPercentage;
        ...
        const grossAmount = invoices.reduce((sum: number, i: { amount: number }) => sum + i.amount, 0);
        ...
        const totalDeductions = commissionAmount + withholdings.reduce((sum, w) => sum + w.amount, 0);
     ...
     */
    async getPendingSettlements(): Promise<Array<{ ownerId: string; period: string }>> {
    ...
    async processSettlement(
        ...
        try {
            ...
            const transferResult = await this.initiateTransfer(settlementId, calculation);

            if (transferResult.success) {
                await this.markSettlementCompleted(settlementId, transferResult.reference!);
                ...
                await this.markSettlementFailed(settlementId, transferResult.error || 'Unknown error');
            ...
            logger.error('Settlement processing failed', { ownerId, period, error: errorMsg });
     ...
     */
    private async upsertSettlementRecord(calculation: SettlementCalculation): Promise<string> {
        const result = await AppDataSource.query(
     ...
     */
    private async markSettlementCompleted(settlementId: string, transferReference: string): Promise<void> {
     ...
     */
    private async markSettlementFailed(settlementId: string, error: string): Promise<void> {
        await AppDataSource.query(
            `UPDATE settlements 
             SET status = 'failed', 
                 notes = $2,
     ...
     */
    async getSettlementHistory(ownerId: string, limit = 12): Promise<Settlement[]> {
        const result = await AppDataSource.query(
