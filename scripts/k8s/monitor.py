...
try:
    ...
    for job in get('jobs')['items']:
        operation=job['spec']['template'].get('metadata',{}).get('labels',{}).get('app')
        if operation in [None,'bootstrap','migrate']:continue
        if operation not in latest_jobs or job['metadata']['creationTimestamp']>latest_jobs[operation]['metadata']['creationTimestamp']:
            latest_jobs[operation]=job
    failed_jobs=sum(any(c['type']=='Failed' and c['status']=='True' for c in j.get('status',{}).get('conditions',[])) for j in latest_jobs.values())
...
metrics={'unhealthy':0 if healthy else 1,'failed_jobs':failed_jobs,'disk_used_percent':100*disk.used/disk.total,'backup_age_seconds':now-last_timestamp if last_timestamp else now-state['started_at']}
