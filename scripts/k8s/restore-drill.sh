...
createdb --owner=rent_user --template=template0 --encoding=UTF8 --locale=C.UTF-8 rent_restore_drill
trap 'dropdb --if-exists rent_restore_drill' EXIT
pg_restore --exit-on-error --dbname=rent_restore_drill "$archive"
psql --dbname=rent_restore_drill -v ON_ERROR_STOP=1 -c 'ALTER TABLE public.spatial_ref_sys OWNER TO rent_user'
